...
class DeviceManager:
    ...
    def genMapping(self):
        ...
        mapping = np.zeros((14,16), dtype=np.int8)
    ...
    def genControlDict(self, mask, control, dictionary_prefix=""):
        ...
        try:
            control_dict.pop(dictionary_prefix + str(0))
        except KeyError:
            pass
...
def maskFromCentroid(shape=(10,12), centroid=(4,5), mask_width=5):
    ...
    n_rows, n_cols = shape
    adjusted_rows, adjusted_cols =  n_rows+4,n_cols+4
    
    x_min = max(0, adjusted_x - floor(mask_width/2))
    x_max = min(adjusted_cols, adjusted_x + ceil(mask_width/2))
    
    y_min = max(0, adjusted_y - floor(mask_width/2))
    y_max = min(adjusted_rows, adjusted_y + ceil(mask_width/2))
    
    mask = np.zeros((adjusted_rows, adjusted_cols))
